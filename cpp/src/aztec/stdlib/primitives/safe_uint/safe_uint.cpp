...
{
    ASSERT(msb >= lsb);
    ASSERT(static_cast<size_t>(msb) <= constants::MAX_NO_WRAP_INTEGER_BIT_LENGTH);
    ...
    // is
    ASSERT(value < (static_cast<uint256_t>(1) << constants::MAX_NO_WRAP_INTEGER_BIT_LENGTH));
    ...
    if (this->value.is_constant()) {
        ...
        hi_wit = safe_uint_t(witness_t(ctx, hi), constants::MAX_NO_WRAP_INTEGER_BIT_LENGTH - uint32_t(msb), "hi_wit");
